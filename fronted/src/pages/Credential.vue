  ...
  <div class="q-pa-md">
    ...
    >
      ...
      <template v-slot:top="props">
        <q-btn color="primary" :label="$t('credential.new')" @click="AddNewCredential" no-caps></q-btn>
        <q-space/>
        <q-input borderless dense debounce="300" :placeholder="$t('Search')" color="primary" v-model="filter" filled>
    ...
    <q-dialog v-model="createcredentialmodal" full-width full-height :maximized="true">
      <q-card>
        ...
        <q-card-section>
            ...
            <div class="q-pa-md">
              ...
              >
                <q-input
                  ...
                  lazy-rules
                  :rules="[ val => val && val.length > 0 || $t('Please type your credential name')]"
                />
                ...
                <q-input
                  ...
                  :hint="$t('User name')"
                  lazy-rules
                  :rules="[ val => val && val.length > 0 || 'Please choose one protocol']"
                />

                <q-toggle v-model="usekey" :label="$t('Use ssh key')" v-show="usekey_show"/>
                ...
                <q-input
                  ...
                  lazy-rules
                  :rules="[
          val => val !== null && val !== '' || $t('Please type a valid port value'),
          val => val <= 65535 || $t('Please type a valid port value')
                ...
                <q-input
                  ...
                  lazy-rules
                  :rules="[
          val => val !== null && val !== '' || $t('Please type a valid width value'),
          val => val >= 0 || $t('Please type a valid width value')
                ...
                <q-input
                  ...
                  lazy-rules
                  :rules="[
          val => val !== null && val !== '' || $t('Please type a valid height value'),
          val => val >= 0 || $t('Please type a valid height value')
                ...
                <q-input
                  ...
                  lazy-rules
                  :rules="[
          val => val !== null && val !== '' || $t('Please type a valid dpi value'),
...
export default {
  ...
  data () {
    return {
      columns: [
        { name: 'name', align: 'center', label: this.$t('credential.table.columns.name.label'), field: 'name', sortable: true },
        { name: 'username', align: 'center', label: this.$t('credential.table.columns.username.label'), field: 'username', sortable: true },
        { name: 'protocol', align: 'center', label: this.$t('credential.table.columns.protocol.label'), field: 'protocol', sortable: true },
        { name: 'port', align: 'center', label: this.$t('credential.table.columns.port.label'), field: 'port', sortable: true }
      ],
      data: [],
    ...
    protocol (value) {
      if (this.create === true) {
        if (value === 'ssh') {
          ...
          this.usekey_show = true
        } else if (value === 'telnet') {
          this.key_show = false
          this.port = 23
          this.width_show = false
          this.height_show = false
          this.usekey_show = false
  ...
  methods: {
    ...
    editRow (props) {
      ...
      if (props.row.proxy_server || props.row.proxy_port || props.row.proxy_password) {
        this.useproxy = true
      }
    ...
    deleteRow (props) {
      this.$q.dialog({
        ...
        message: `${this.$t('credential.delete_credential', { name: props.row.name })} ?`,
        cancel: true,
        persistent: true,
        ok: {
          push: true,
    ...
    filterSecurityFn (val, update) {
      ...
      update(() => {
        const needle = val.toLowerCase()
        this.security_options = securityOptions.filter(v => v.toLowerCase().indexOf(needle) > -1)
      })
    },
    fetchData () {
      const that = this
    ...
    createCredential (data) {
      const that = this
      this.$axios.post('/common/api/credential/', data).then(res => {
        that.$q.notify({
          type: 'positive',
          textColor: 'grey-10',
          multiLine: true,
          message: `${that.$t('credential.create_credential_success', { name: data.name })} !`,
          timeout: 2000,
          position: 'top'
        })
    ...
    deleteCredential (id, name) {
      const that = this
      this.$axios.delete(`/common/api/credential/${id}/`).then(res => {
        that.$q.notify({
          type: 'positive',
          textColor: 'grey-10',
          multiLine: true,
          message: `${that.$t('credential.delete_credential_success', { name: name })} !`,
          timeout: 2000,
          position: 'top'
        })
        that.fetchData()
    ...
    updateCredential (id, data) {
      const that = this
      this.$axios.patch(`/common/api/credential/${id}/`, data).then(res => {
        that.$q.notify({
          type: 'positive',
          textColor: 'grey-10',
          multiLine: true,
          message: `${that.$t('credential.update_credential_success', { name: that.name })} !`,
          timeout: 2000,
          position: 'top'
        })
        that.createcredentialmodal = false
...
<style scoped>
  /*@import "../css/bootstrapcustom.min.css";*/
