...
class ElfinderConnector:
    ...
    def execute(self, cmd, **kwargs):
        ...
        if 'removed' in result:
            ...
            result['removed'] = list(
                set([f['hash'] for f in result['removed']]))
        ...
        if cmd != u'file' and 'spdfid_' in self._volumes.keys():
            self._volumes['spdfid_']._options['storage'].sftp.sock.transport.close()
        ...
        **Command**: Open a directory
        ...
        try:
            ...
            if not cwd['read'] and init:
                try:
                    cwd = volume.dir(
                        hash_=volume.default_path(), resolve_link=True)
            ...
            for dirs in name:
                try:
                    ...
                    result['warning'] = self.error(
                        ElfinderErrorMessages.ERROR_UPLOAD_FILE, dirs, e)
            return result
        ...
        **Command**: Duplicate a file. Create a copy with "{suffix} %d" suffix,
        ...
        for target in targets:
            try:
                ...
                result['warning'] = self.error(
                    ElfinderErrorMessages.ERROR_COPY, u'#%s' % target, ElfinderErrorMessages.ERROR_FILE_NOT_FOUND)
                continue
        ...
        **Command**: Remove directories or files. This method should not be invoked 
        ...
        for target in targets:
            try:
                ...
                result['warning'] = self.error(
                    ElfinderErrorMessages.ERROR_RM, '#%s' % target, ElfinderErrorMessages.ERROR_FILE_NOT_FOUND)
                continue
        ...
        **Command**: Save uploaded files. This method should not be invoked 
        ...
        if not upload_path:  # not is directory
            for uploaded_file in files:
                try:
                    file_ = volume.upload(
                        uploaded_file, target, chunk_name, is_first_chunk)
            ...
            try:
                ...
                for key, value in [(v, i) for i, v in enumerate(upload_path)]:
                    if key.startswith('/'):
            ...
            for item in all_.keys():
                # get real path
                real_path = "%s/%s" % (volume.decode(target), item)
                ...
                for file_index in all_[item]:
                    try:
                        file_ = volume.upload(
                            files[file_index], new_target, chunk_name, is_first_chunk)
                        result['added'].append(file_)
                    except Exception as e:
                        result['warning'] = self.error(
                            ElfinderErrorMessages.ERROR_UPLOAD_FILE, uploaded_file.name, e)
        ...
        **Command**: Copy/move ``targets`` files into a new destination ``dst``.
        ...
        for target in targets:
            try:
                ...
                result['warning'] = self.error(
                    error, u'#%s' % target, ElfinderErrorMessages.ERROR_FILE_NOT_FOUND)
                continue

            try:
                result['added'].append(
                    dstVolume.paste(srcVolume, target, dst, cut))
            except NamedError as e:
                result['warning'] = self.error(e, e.name)
            except Exception as e:
                result['warning'] = self.error(e)
    ...
    def _resize(self, target, width, height, mode=None, x='0', y='0', degree='0'):
        ...
        **Command**: Resize ``target`` image. This method should not be invoked 
        ...
        method must be used.
        """
        width, height, x, y, degree = int(width), int(
            height), int(x), int(y), int(degree)
        bg = ''
