...
class ElfinderVolumeStorage(ElfinderVolumeDriver):
    ...
    def mount(self, opts):
        ...
        if not 'storage' in opts:
            if not 'storageClass' in opts:
                ...
                opts['storage'] = opts['storageClass'](
                    **opts['storageKwArgs'])
                ...
                try:
                    ...
                    raise Exception(
                        'Storage %s should implement both the listdir() and url() methods to be valid for use with yawd-elfinder.' % self._options['storage'].__class__)
    ...
    def _configure(self):
        ...
        if not self._isabs(self._options['tmbPath']):
            ...
            if not self._options['tmbURL'] and self._options['URL']:
                self._options['tmbURL'] = self._options['URL'] + self._options['tmbPath'][len(
                    self._root) + 1:].replace(self._separator, '/') + '/'
        ...
        if not 'rmDir' in self._options or not callable(self._options['rmDir']):
            ...
            # cancel delete disable directory
    ...
    def _get_available_name(self, dir_, name, ext, i):
        ...
        """
        path = self._options['storage'].get_available_name(
            self._join_path(dir_, '%s%s' % (name, ext)))
    ...
    def _stat(self, path):
        ...
        try:
            stat['ts'] = time.mktime(
                self._options['storage'].modified_time(path).timetuple())
        except NotImplementedError:
            stat['ts'] = ''
    ...
    def _copy(self, source, target_dir, name):
        ...
        self._options['storage'].save(self._join_path(
            target_dir, name), DjangoFile(tmp_file))
        tmp_file.close()
    ...
    def _archive(self, dir_, files, name, arc):
        ...
        quarantine_dir = os.path.join(
            self._options['quarantine'], '%s-temp' % name)
    ...
    def _local_file_mimetype(self, path, name=''):
        ...
        mime = magic.Magic(mime=True).from_file(
            path.encode('utf-8'))  # unicode filename support
        int_mime = None
    ...
    def _move_from_local(self, path, dst, name):
        ...
        if os.path.isdir(path):
            for p in map(lambda x: os.path.join(path, x), os.listdir(path)):
                self._move_from_local(p, self._join_path(
                    dst, name), os.path.basename(p))
            shutil.rmtree(path)
        else:
