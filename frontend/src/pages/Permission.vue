...
export default {
  ...
  methods: {
    ...
    deleteRow (props) {
      ...
      this.$q
        .dialog({
          ...
          message: `${this.$t('permission.delete_permission', {
            name: props.row.user.username
          })} ?`,
          cancel: true,
          persistent: true,
          ok: {
    ...
    fetchdata () {
      ...
      that.loading = true
      this.$axios
        .get('/permission/api/permissionwithinfo/')
        .then(res => {
          that.data = res.data
          that.loading = false
        })
    ...
    deletePermission (id, name) {
      const that = this
      this.$axios
        .delete(`/permission/api/permission/${id}/`)
        .then(res => {
          that.$q.notify({
            type: 'positive',
            textColor: 'grey-10',
            multiLine: true,
            message: `${that.$t('permission.delete_permission_success', {
              name: name
            })} !`,
            timeout: 2000,
            position: 'top'
    ...
    createPermission (data) {
      const that = this
      this.$axios
        .post('/permission/api/permission/', data)
        .then(res => {
          that.$q.notify({
            type: 'positive',
            textColor: 'grey-10',
            multiLine: true,
            message: `${that.$t('permission.create_permission_success', {
              name: data.name
            })} !`,
            timeout: 2000,
            position: 'top'
    ...
    updatePermission (id, data) {
      const that = this
      this.$axios
        .patch(`/permission/api/permission/${id}/`, data)
        .then(res => {
          that.$q.notify({
            type: 'positive',
            textColor: 'grey-10',
            multiLine: true,
            message: `${that.$t('permission.update_permission_success', {
              name: data.name
            })} !`,
            timeout: 2000,
            position: 'top'
    ...
    filterCredential (val, update) {
      update(() => {
        ...
        if (val === '') {
          this.credentials_list = credentialsListTmp
        } else {
          const needle = val.toLowerCase()
          this.credentials_list = credentialsListTmp.filter(
            v => v.label.toLowerCase().indexOf(needle) > -1
          )
        }
      })
