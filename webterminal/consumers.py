...
class CommandExecute(WebsocketConsumer, WebsocketAuth):
    ...
    def receive(self, text=None, bytes=None, **kwargs):
        try:
            if text:
                ...
                if data.has_key('parameter'):
                    ...
                    if taskname and ip and groupname:
                        ...
                        server_list = []
                        [server_list.extend([server.ip for server in ServerGroup.objects.get(name=group.name).servers.all(
                        )]) for group in CommandsSequence.objects.get(name=taskname).group.all()]
                    elif taskname and groupname and not ip:
                        server_list = [server.ip for server in ServerGroup.objects.get(
                            name=groupname).servers.all()]
                    commands = json.loads(
                        CommandsSequence.objects.get(name=taskname).commands)
                    if isinstance(commands, (basestring, str, unicode)):
                        commands = ast.literal_eval(commands)

                    # Run commands
                    commandshell = ShellHandlerThread(
                        message=self.message, commands=commands, server_list=set(server_list))
                    ...
                    # illegal
                    self.message.reply_channel.send({"text": json.dumps(
                        ['stdout', '\033[1;3;31mIllegal parameter passed to the server!\033[0m'])}, immediately=True)
            ...
            logger.error(
                "Some error happend, Please report it to the administrator! Error info:{0}".format(e))
